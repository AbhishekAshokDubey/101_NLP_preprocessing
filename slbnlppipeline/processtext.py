...
def pipeline(config_file_path, sequence=[]):
    ...
    text_list = readfile(cfg['readfile']['path'], cfg['readfile']['column_info'])
        ...
        if rank > 0:
            ...
            if operation == 'stopword_removal':
                if cfg['stopword_removal']['stopwords_file_path'].strip() == '':
                    text_list = removestopword(text_list, stopword_list = stopwords.words('english'))
                else:
                    ...
                    text_list = removestopword(text_list, stopwords_list)

            if operation == 'dict_replacement':
                text_list = dict_replace(text_list, regex_dict_path=cfg['dict_replacement']['regex_dict_path'].strip(), word_dict_path=cfg['dict_replacement']['word_dict_path'].strip())
            if operation == 'spell_correction':
                text_list = spellcorrection(text_list, spell_corrector = cfg['spell_correction']['spell_corrector_type'].strip(), dictpath=cfg['spell_correction']['spell_list_hunspell'].strip())
            if operation == 'stemming':
                text_list = stem(text_list, stemmertype = cfg['stemming']['stemmertype'])
            if operation == 'lemmatizer':
                text_list = lemmatize(text_list, lemmatizertype = cfg['lemmatizer']['lemmatizertype'])
            if operation == 'postagging':                
                text_list = postagging(text_list, tagger = cfg['postagging']['tagger'])
            if operation == 'word2vec_model':
                if cfg['word2vec_model']['train_textcorpus'] == 'samedata':
                    return word2vec_model(text_list, word2vec_algo=cfg['word2vec_model']['word2vec_algo'])
                else:
                    return word2vec_model(cfg['word2vec_model']['train_textcorpus'], word2vec_algo=cfg['word2vec_model']['word2vec_algo'])                    
